...
class AppIndicatorExample:
    ...
    def __init__(self):
        ...
        #    print('Got pa-server bus from dbus: %s', srv_addr)
            self.conn = self.connect()
            self.core = self.conn.get_object(object_path="/org/pulseaudio/core1")
            print "Successfully connected to " + self.core.Get("org.PulseAudio.Core1", "Name", dbus_interface="org.freedesktop.DBus.Properties") + "!"
    ...
    def action(self):
        print "SINKS:"
        for sink in self.core.Get("org.PulseAudio.Core1", "Sinks", dbus_interface="org.freedesktop.DBus.Properties"):
            print sink
        ...
        print
        print "SOURCES:"
        for source in self.core.Get("org.PulseAudio.Core1", "Sources", dbus_interface="org.freedesktop.DBus.Properties"):
            print source
            s = self.conn.get_object(object_path=source)
            print s.Get("org.PulseAudio.Core1.Device", "Name", dbus_interface="org.freedesktop.DBus.Properties")
            #print
        print "PLAYBACKSTREAMS:"
        for pstream in self.core.Get("org.PulseAudio.Core1", "PlaybackStreams", dbus_interface="org.freedesktop.DBus.Properties"):
            print pstream
            ...
            #print propertyList
            # [:-1] because the strings containd zerobytes at the end
            appName = ''.join([chr(character) for character in propertyList["application.name"]])[:-1]
            mediaName = ''.join([chr(character) for character in propertyList["media.name"]])[:-1]
            print "%s: %s\n" % (appName, mediaName)
